...
function createSfx() {
  ...
  return { cardDeal, cardFlip, keyTap, keyBackspace };
}
...
export default function HomePage() {
  ...
  const unlock = () => {
    try {
      ...
      sessionStorage.setItem("ridbusters.lastView", "landing");
    } catch {}
    setLocked(false);
    setCode("");
    setCodeErr(null);
  ...
  useEffect(() => {
    const forceReloadLock = isReloadNavigation();

    if (forceReloadLock) {
      try {
        sessionStorage.removeItem("ridbusters.unlocked");
        sessionStorage.removeItem("ridbusters.skipIntroOnce");
        sessionStorage.removeItem("ridbusters.lastView");
        sessionStorage.removeItem("ridbusters.fromChatBack");
      } catch {}
      setLocked(true);
      setView("landing");
      setReady(true);
    ...
    let unlocked = false;
    try {
      unlocked = sessionStorage.getItem("ridbusters.unlocked") === "1";
    } catch {}
          ...
          {locked && !forceAvatars ? (
            <div className="rbLockLayer" role="dialog" aria-modal="true" onPointerDown={focusHidden}>
              <div className="rbLockCard" onPointerDown={focusHidden}>
                ...
                <button type="button" className="rbEnterBtn" onClick={checkCode}>
                  ENTER
                </button>
              </div>
            </div>
